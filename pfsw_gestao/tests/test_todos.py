...
from pfsw_gestao.models.models import Todo, TodoState
...
class TestTodoEndpoint:
    def test_create_todo(self, client, token):
    ...
    async def test_list_todos_pagination_should_return_2_todos(
        self, session, user, client, token
    ):
        expected_todos = 2
        session.add_all(TodoFactory.create_batch(5, user_id=user.id))
        await session.commit()
    ...
    def test_patch_todo_error(self, client, token):
    ...
    async def test_patch_todo(
        self, session, client, user, token
    ):
    ...
    async def test_delete_todo(
        ...
        assert response.status_code == HTTPStatus.OK
        assert response.json() == {
            'message': 'Task foi excluída com sucesso.'
        }

    def test_delete_todo_error(self, client, token):
        response = client.delete(
